...
#+begin_src emacs-lisp
;; Ag(silver searcher)
;; Projectile integrated key is `C-c p s s`
(use-package ag
  :ensure t
  :custom
  (ag-highlight-search t)
  (ag-reuse-buffers t)) ;; Use the same buffer to display results

(use-package wgrep
;; Edit in search buffer and apply to files
  :ensure t) ;; wgrep-change-to-wgrep-mode
#+end_src
* Git timemachine
#+begin_src emacs-lisp
